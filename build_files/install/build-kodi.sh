...
install_ffmpeg_headers() {
    ...
    # Extract headers with proper directory structure
    log_info "Extracting headers to $HEADER_DIR..."
    tar -xf "ffmpeg-${FFMPEG_VERSION}.tar.xz" --strip-components=1 \
        -C "$HEADER_DIR" \
        "ffmpeg-${FFMPEG_VERSION}/libavcodec" \
        "ffmpeg-${FFMPEG_VERSION}/libavformat" \
        "ffmpeg-${FFMPEG_VERSION}/libavutil" \
        "ffmpeg-${FFMPEG_VERSION}/libavfilter" \
        "ffmpeg-${FFMPEG_VERSION}/libswscale" \
        "ffmpeg-${FFMPEG_VERSION}/libswresample" \
        "ffmpeg-${FFMPEG_VERSION}/libpostproc" \
        "ffmpeg-${FFMPEG_VERSION}/libavdevice" \
        --wildcards "*/\.h" \
        2>/dev/null || true

    # Also need config headers from the build
    # Create a minimal avconfig.h if it doesn't exist
    if [ ! -f "$HEADER_DIR/libavutil/avconfig.h" ]; then
        log_info "Creating avconfig.h..."
        cat > "$HEADER_DIR/libavutil/avconfig.h" << 'EOF'
...
EOF
    ...
    # Fix the pkg-config files to point to correct location
    local PC_DIR="/tmp/ffmpeg-pkgconfig"
    for pc in "$PC_DIR"/*.pc; do
        sed -i "s|includedir=.*|includedir=/usr/include|" "$pc"
    done

    log_success "FFmpeg headers installed with correct structure"
}
