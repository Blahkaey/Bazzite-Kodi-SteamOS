...
install_kodi_pre_launch_script() {
    ...
    cat > "/usr/bin/kodi-pre-launch" << 'EOF'
#!/bin/bash
#
...
echo "[KODI-PRELAUNCH] Running pre-launch setup as root..."
echo "[KODI-PRELAUNCH] $(date)"

# Ensure we're on the right TTY
chvt 1 2>/dev/null || true
...
for attempt in 1 2 3; do
    ...
    if set_drm_content_type 2; then
...
# Run pre-launch script as root to set DRM properties
ExecStartPre=/usr/bin/kodi-pre-launch
